...
bool Loader::LoadDriver()
{
	...
	{
		system("color c");
		cout << "-- Driver yukleme basarisiz oldu. Hata kodu: 0x" << hex << ret << dec << endl;
		cout << "---- 0xc0000035: Proxy driver zaten yukludur. 35 Haricinde ne ile bitiyorsa, virus programini ve secureboot'u kapatip deneyin.";
		getchar();
		exit(-1);
		return false;
	}
...
bool Loader::UnLoadDriver()
{
	...
	{
		system("color c");
		cout << "-- Driver unload basarisiz. Hata kodu: 0x" << hex << ret << dec << endl;
		cout << "----- 0xc0000010: Bilgisayarinizi yeniden baslatip tekrar deneyin!" << endl;
		cout << "----- 0xc0000061: Proxy driver yuklenemedi." << endl;
		cout << "----- 0xc0000035: Driver zaten unload edildi." << endl;
		getchar();
		exit(-1);
	}
...
bool Loader::DeleteRegistryKey()
{
	...
	if (ret)
		cout << "Kayit anahrati silinemedi. Hata kodu: 0x" << hex << ret << dec << endl;
