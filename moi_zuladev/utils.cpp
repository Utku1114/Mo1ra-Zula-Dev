...
uint64_t utils::GetKernelModuleAddress(const std::string& module_name)
{
	...
	NTSTATUS status = NtQuerySystemInformation(static_cast<SYSTEM_INFORMATION_CLASS>(nt::SystemModuleInformation2), buffer, buffer_size, &buffer_size);
	...
	while (status == nt::STATUS_INFO_LENGTH_MISMATCH)
	{
		...
		status = NtQuerySystemInformation(static_cast<SYSTEM_INFORMATION_CLASS>(nt::SystemModuleInformation2), buffer, buffer_size, &buffer_size);
	}
