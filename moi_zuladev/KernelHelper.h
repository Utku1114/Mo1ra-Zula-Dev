...
NON_PAGED_DATA static UCHAR Kh_PassiveCallStubData[] =
{
	...
	0x8F, 0x40, 0x12,                                // pop    QQWORD PTR [rax+0x12]  ; ret_store
	0x50,                                            // push rax
...
static void Khk_FreePassiveStub(CapcomContext* CpCtx, KernelContext* KrCtx)
{
	if (!Khk_PassiveCallStub)
		return;

	NON_PAGED_DATA static auto Khk_ExFreePoolWithTag = KrCtx->GetProcAddress<fnFreeCall>("ExFreePoolWithTag");
	assert(Khk_ExFreePoolWithTag);

	CpCtx->ExecuteInKernel(NON_PAGED_LAMBDA() {
		Khk_ExFreePoolWithTag(Khk_PassiveCallStub, 0);
	});
}